...
def download_from_instagram(hastag_mickey_feio):
    """
    Função que faz o donwload de todas as imagens do instagram com a hastag
    """
    loader = instaloader.Instaloader()
    # loader.interactive_login(user)

    hashtag = instaloader.Hashtag.from_name(loader.context, hastag_mickey_feio)

    for post in hashtag.get_posts():
        loader.download_post(post, target=f"#{hashtag.name}")
...
def delete_unused_files():
    ...
    fileListTxt = glob.glob(f"#{hastag_mickey_feio}{separador}*.txt")
    fileListJson = glob.glob(f"#{hastag_mickey_feio}{separador}*.json")
...
def extract_info():
    ...
    """
    fileListXz = glob.glob(f"#{hastag_mickey_feio}{separador}*.xz")
    dados_list = []
...
def rename_files():
    ...
    adicionando o nome do autor da foto, no nome do arquivo de imagem.
    """
    ...
    for image in os.scandir(f"#{hastag_mickey_feio}"):
        image = image.path
...
if __name__ == '__main__':
    ...
    hastag_mickey_feio = f"mickeyfeio{year}"

    download_from_instagram(hastag_mickey_feio)
    delete_unused_files()
    rename_files()
